    ...
    <div>
        ...
        <PageSection
            ...
            <CategoryCarousel
                ...
                :filter-categories="[currentCategoryId]"
                urlPrefix="/inspiration/images/category/"
                linkToLabel="View gallery"
            />
        </PageSection>
        <PageSectionSeparator/>
        <PageSection without-top-body-margin without-vertical-margins :style="photoLibarySectionStyle">
            <PhotoLibrary
...
export default {
    ...
    computed: {
        ...
        title() {
            ...
            if (category) {
                const categoryData = this.categories.find(c => c.slug === category)
                if (categoryData) {
                    return categoryData.page_title || categoryData.name + ' Photos'
                }
            }
            return 'Photo Library Search Results:'
        },
        currentCategoryId() {
    ...
    methods: {
        ...
        async loadMoreImages({direction}) {
            ...
            const paramsParsed = createParamsString(params)
            const portfolioImages = await this.$http.$get(`galleries_search/${paramsParsed}`)
            await this.$store.dispatch('api/galleries/addImages', portfolioImages.items)
            this.currentPage++
            this.loading = false
        }
    },
