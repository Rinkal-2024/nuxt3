    ...
    <div class="card-style-4">
        ...
        <template v-if="image">
            <Photo
                ...
                withPhotoCredits
            />
            <AmpPhoto v-else :height="ampPhotoHeight" :image="image"/>
        </template>

        <h2 class="title">
            <Link :rel="noFollow ? 'nofollow' : false" :to="linkTo" class="title__link js-card-style-4-link-title"
                  withoutStyles>
                {{ title }}
            </Link>
        </h2>
...
export default {
    ...
    computed: {
        category() {
            //const categoriesString = this.categories.reduce((categoriesString, category) => {
            //    categoriesString += category['name'] ?? category + ' & '
            //}, '')

            return this.categories[0]?.name
        },
        truncatedDescription() {
            const maxDescriptionLength = 95
            if (this.description.length <= maxDescriptionLength) {
                return this.description
            }
            return this.description.slice(0, maxDescriptionLength) + '...'
        }
    }
}
</script>
