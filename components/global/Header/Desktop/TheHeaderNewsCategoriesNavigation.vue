<template>
    <nav class="news-categories-navigation" role="navigation">
        <div
            ...
            class="link-wrapper"
        >
            <Link :to="linkToCategory(category.categorySlug)" class="link">
                {{ category.name }}
            </Link>
...
export default {
    name: 'TheHeaderNewsCategoriesNavigation',
    data: () => ({
        NEWS_CATEGORIES
    }),
    methods: {
        linkToCategory(categorySlug) {
            return {
                name: 'news-category',
                params: {
        ...
        subcategoryLinkTo(categorySlug, subcategorySlug) {
            return {
