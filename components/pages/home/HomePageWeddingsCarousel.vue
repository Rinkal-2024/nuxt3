<template>
    <div>
        <div v-if="!$isAMP" id="weddings-slider" class="slider">
            <div class="slide" v-for="(item, index) in items" :key="item.id" :style="calcStyle(index)">
                 <Link :to="weddingLink(item)">
                    <Photo
                        :image="item.image"
                        :isSharable="false"
                        :lazy="true"
...
export default {
    name: 'HomePageSecondaryCarousel',
    computed: {
        ...mapGetters('home/weddingsCarousel', [
            'items'
        ]),
         sizes() {
    ...
    methods: {
        weddingLink(wedding) {
            return `/weddings/${wedding.linkTo}/${wedding.id}`
        },
        calcStyle(index) {
