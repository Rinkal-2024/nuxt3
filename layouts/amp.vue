<template>
    <amp-body>
        <amp-sidebar id="sidebar" class="main-sidebar" layout="nodisplay" side="left">
            <div class="drawer__inner-container">
                <span class="drawer__title">Navigation</span>
                <NuxtLink
                    v-for="pageLink in pageLinks"
                    v-show="!pageLink.onlyDesktop"
                    :key="pageLink.name"
                    :to="pageLink.to"
                    class="link">
                    {{ pageLink.name }}
                </NuxtLink>
                <span class="drawer__title">News categories</span>
                <NuxtLink
                    v-for="category in NEWS_CATEGORIES"
                    :key="category.name"
                    :to="linkToCategory(category.categorySlug)"
                    class="link">
                    {{ category.name }}
                </NuxtLink>
                <span class="drawer__title"></span>
                <div class="main-navigation-container">
                    <nav class="main-navigation">
                        <NuxtLink
                            :to="{ name: 'about' }"
                            class="link"
                        >
                            About
                        </NuxtLink>
                        <NuxtLink
                            :to="{ name: 'advertise' }"
                            class="link"
                        >
                            Advertise
                        </NuxtLink>
                        <NuxtLink
                            :to="{ name: 'contact' }"
                            class="link"
                        >
                            Contact
                        </NuxtLink>
                        <a
                            class="link"
                            href="https://simplecirc.com/subscribe/inside-weddings"
                        >
                            Subscribe to Magazine
                        </a>
                        <Link without-styles to="/newsletter/" force-no-amp class="link">
                            Subscribe to Newsletter
                        </Link>
                        <NuxtLink
                            :to="{ name: 'wedding-submit' }"
                            class="link"
                        >
                            Submit a Wedding
                        </NuxtLink>
                        <NuxtLink
                            :to="{ name: 'work-with-us' }"
                            class="link"
                        >
                            Are You a Business?
                        </NuxtLink>
                    </nav>
                </div>
                <SocialLinks/>
            </div>
        </amp-sidebar>
        <!--        <button on="tap:sidebar.toggle">Toggle sidebar</button>-->
        <!--        <button on="tap:sidebar.open">Open sidebar</button>-->
        <header class="amp-headerbar">
            <div class="hamburger" on="tap:sidebar.toggle" role="button" tabindex="0">
                <amp-img :src="getIcon('hamburger')" height="25" noloading width="25"/>
            </div>
            <div class="logo">
                <NuxtLink
                    class="logo__a"
                    to="/amp/"
                >
                    <svg height="21px" version="1.1" viewBox="0 0 470 40" width="208px"
                         xmlns="http://www.w3.org/2000/svg"
                         xmlns:xlink="http://www.w3.org/1999/xlink">
                        <g id="Desktop" fill="none" fill-rule="evenodd" stroke="none"
                           stroke-width="1">
                            <g id="IW_Homepage_1.1" fill="#000000"
                               fill-rule="nonzero" transform="translate(-63.000000, -40.000000)">
                                <g id="Header" transform="translate(63.000000, 40.000000)">
                                    <g id="IW_logo">
                                        <polygon id="Path"
                                                 points="0 38.2857143 5.9350365 38.2857143 5.9350365 1.71428571 0 1.71428571 0 0.96 17.2218978 0.96 17.2218978 1.71428571 11.2868613 1.71428571 11.2868613 38.2857143 17.2218978 38.2857143 17.2218978 39.0342857 0 39.0342857"></polygon>
                                        <polygon id="Path"
                                                 points="15.2950122 38.2857143 21.2300487 38.2857143 21.2300487 1.71428571 15.2950122 1.71428571 15.2950122 0.96 26.7305353 0.96 44.8043796 30.1028571 44.913017 30.1028571 44.913017 1.71428571 38.9894161 1.71428571 38.9894161 0.96 51.6656934 0.96 51.6656934 1.71428571 45.7420925 1.71428571 45.7420925 39.7714286 44.6557178 39.7714286 22.1391727 3.90285714 22.036253 3.90285714 22.036253 38.2857143 27.9712895 38.2857143 27.9712895 39.0342857 15.2950122 39.0342857"></polygon>
                                        <path
                                            id="Path"
                                            d="M51.9858881,27.5257143 L52.5576642,27.5257143 C55.0163017,39.1257143 61.5974453,39.0742857 65.5541363,39.0742857 C73.0958637,39.0742857 75.0227494,33.8857143 75.0227494,30.4114286 C75.0227494,25.44 70.8487835,23.4114286 67.8526764,22.5028571 L61.7175182,20.6285714 C55.9425791,18.8571429 52.946472,16.0857143 52.946472,11.2228571 C52.946472,4.75428571 57.7493917,0 64.6621655,0 C69.636618,0 71.0260341,2.08571429 73.0558394,2.08571429 C73.9649635,2.08571429 74.1307786,1.97714286 74.6625304,0.942857143 L75.4115572,0.942857143 L75.4115572,10.6342857 L74.7711679,10.6342857 C73.9135036,5.49142857 70.7687348,0.748571429 64.6621655,0.748571429 C61.1857664,0.748571429 55.8396594,2.40571429 55.8396594,8.06857143 C55.8396594,12.6685714 59.8420925,14.2171429 63.5357664,15.3942857 L70.0082725,17.4228571 C72.6841849,18.28 79.156691,19.8857143 79.156691,27.2057143 C79.156691,33.8342857 74.0107056,39.8171429 65.5712895,39.8171429 C63.2963701,39.8996061 61.0223824,39.6297463 58.8300487,39.0171429 C57.4406326,38.5885714 55.6223844,37.84 54.3930657,37.84 C53.3238443,37.84 53.0551095,38.3257143 52.7863747,39.0171429 L51.9858881,39.0171429 L51.9858881,27.5257143 Z"></path>
                                        <polygon id="Path"
                                                 points="80.620438 38.2857143 86.5554745 38.2857143 86.5554745 1.71428571 80.620438 1.71428571 80.620438 0.96 97.836618 0.96 97.836618 1.71428571 91.8844282 1.71428571 91.8844282 38.2857143 97.8194647 38.2857143 97.8194647 39.0342857 80.620438 39.0342857"></polygon>
                                        <path
                                            id="Shape"
                                            d="M96.3785888,38.2857143 L102.313625,38.2857143 L102.313625,1.71428571 L96.3785888,1.71428571 L96.3785888,0.96 L106.350365,0.96 C107.362409,0.96 108.32871,0.908571429 109.346472,0.857142857 C110.364234,0.805714286 111.376277,0.8 112.336861,0.8 C116.670925,0.8 118.809367,1.06857143 121.754015,2.02857143 C130.467883,4.88571429 134.264477,11.92 134.264477,19.9885714 C134.264477,29.3428571 129.398662,34.9542857 123.252068,37.5771429 C119.775669,39.0742857 117.534307,39.2342857 112.496959,39.2342857 C111.427737,39.2342857 110.209854,39.1771429 108.9691,39.1257143 C107.728345,39.0742857 106.561922,39.0171429 105.492701,39.0171429 L96.4014599,39.0171429 L96.3785888,38.2857143 Z M107.66545,38.2857143 C108.894769,38.3942857 110.495742,38.5028571 112.422628,38.5028571 C117.288443,38.5028571 122.051338,37.7542857 125.047445,32.7257143 C127.33455,28.88 128.095012,24.9771429 128.095012,20.2228571 C128.095012,8.94285714 124.189781,1.56571429 112.211071,1.56571429 C110.444282,1.56571429 108.997689,1.67428571 107.66545,1.72571429 L107.66545,38.2857143 Z"></path>
                                        <path
                                            id="Path"
                                            d="M135.156448,38.2857143 L141.091484,38.2857143 L141.091484,1.71428571 L135.156448,1.71428571 L135.156448,0.96 L165.746472,0.96 L165.746472,10.2114286 L165.174696,10.2114286 C163.339294,4.65142857 161.200852,1.71428571 155.317275,1.71428571 L146.437591,1.71428571 L146.437591,18.2285714 L149.91399,18.2285714 C154.676886,18.2285714 156.174939,16.7828571 157.724453,10.3714286 L158.296229,10.3714286 L158.296229,26.9942857 L157.724453,26.9942857 C156.763869,22.8742857 155.906204,18.9942857 150.771655,18.9942857 L146.437591,18.9942857 L146.437591,38.2857143 L156.546594,38.2857143 C164.357056,38.2857143 166.764234,30.5885714 167.193066,27.3314286 L167.764842,27.3314286 L167.764842,39.0171429 L135.156448,39.0171429 L135.156448,38.2857143 Z"></path>
                                        <polygon id="Path"
                                                 points="183.540146 0.96 199.635645 0.96 199.635645 1.71428571 194.060827 1.71428571 205.078954 32.7142857 205.187591 32.7142857 212.300487 12.9885714 208.235158 1.71428571 203.740998 1.71428571 203.740998 0.96 219.790754 0.96 219.790754 1.71428571 213.958637 1.71428571 224.976764 32.7142857 225.085401 32.7142857 236.315085 1.71428571 230.597324 1.71428571 230.597324 0.96 242.032847 0.96 242.032847 1.71428571 237.218491 1.71428571 223.370073 39.8171429 222.032117 39.8171429 212.837956 14.1657143 212.729319 14.1657143 203.420803 39.8171429 201.979927 39.8171429 188.343066 1.71428571 183.540146 1.71428571"></polygon>
                                        <path
                                            id="Path"
                                            d="M238.8309,38.2857143 L244.765937,38.2857143 L244.765937,1.71428571 L238.8309,1.71428571 L238.8309,0.96 L269.420925,0.96 L269.420925,10.2114286 L268.849148,10.2114286 C267.019465,4.65142857 264.875304,1.71428571 259.014599,1.71428571 L250.117762,1.71428571 L250.117762,18.2285714 L253.594161,18.2285714 C258.351338,18.2285714 259.849392,16.7828571 261.398905,10.3714286 L261.970681,10.3714286 L261.970681,26.9942857 L261.398905,26.9942857 C260.438321,22.8742857 259.580657,18.9942857 254.446107,18.9942857 L250.117762,18.9942857 L250.117762,38.2857143 L260.221046,38.2857143 C268.031509,38.2857143 270.450122,30.5714286 270.867518,27.3142857 L271.439294,27.3142857 L271.439294,39.0171429 L238.8309,39.0171429 L238.8309,38.2857143 Z"></path>
                                        <path
                                            id="Shape"
                                            d="M272.205474,38.2857143 L278.140511,38.2857143 L278.140511,1.71428571 L272.205474,1.71428571 L272.205474,0.96 L282.15438,0.96 C283.172141,0.96 284.132725,0.908571429 285.150487,0.857142857 C286.168248,0.805714286 287.180292,0.8 288.140876,0.8 C292.474939,0.8 294.613382,1.06857143 297.558029,2.02857143 C306.271898,4.88571429 310.068491,11.92 310.068491,19.9885714 C310.068491,29.3428571 305.202676,34.9542857 299.056083,37.5771429 C295.579684,39.0742857 293.338321,39.2342857 288.306691,39.2342857 C287.231752,39.2342857 286.019586,39.1771429 284.773114,39.1257143 C283.526642,39.0742857 282.365937,39.0171429 281.296715,39.0171429 L272.205474,39.0171429 L272.205474,38.2857143 Z M283.492336,38.2857143 C284.721655,38.3942857 286.322628,38.5028571 288.249513,38.5028571 C293.115328,38.5028571 297.878224,37.7542857 300.874331,32.7257143 C303.161436,28.88 303.921898,24.9771429 303.921898,20.2228571 C303.921898,8.94285714 300.016667,1.56571429 288.037956,1.56571429 C286.271168,1.56571429 284.830292,1.67428571 283.492336,1.72571429 L283.492336,38.2857143 Z"></path>
                                        <path
                                            id="Shape"
                                            d="M311.515085,38.2857143 L317.455839,38.2857143 L317.455839,1.71428571 L311.515085,1.71428571 L311.515085,0.96 L321.46399,0.96 C322.481752,0.96 323.442336,0.908571429 324.460097,0.857142857 C325.477859,0.805714286 326.489903,0.8 327.456204,0.8 C331.78455,0.8 333.92871,1.06857143 336.86764,2.02857143 C345.587226,4.88571429 349.38382,11.92 349.38382,19.9885714 C349.38382,29.3428571 344.518005,34.9542857 338.365693,37.5771429 C334.889294,39.0742857 332.647932,39.2342857 327.616302,39.2342857 C326.54708,39.2342857 325.329197,39.1771429 324.082725,39.1257143 C322.836253,39.0742857 321.681265,39.0171429 320.606326,39.0171429 L311.515085,39.0171429 L311.515085,38.2857143 Z M322.801946,38.2857143 C324.031265,38.3942857 325.660827,38.5028571 327.559124,38.5028571 C332.430657,38.5028571 337.187835,37.7542857 340.183942,32.7257143 C342.471046,28.88 343.231509,24.9771429 343.231509,20.2228571 C343.231509,8.94285714 339.326277,1.56571429 327.347567,1.56571429 C325.580779,1.56571429 324.139903,1.67428571 322.773358,1.72571429 L322.801946,38.2857143 Z"></path>
                                        <polygon id="Path"
                                                 points="350.990511 38.2857143 356.925547 38.2857143 356.925547 1.71428571 350.990511 1.71428571 350.990511 0.96 368.223844 0.96 368.223844 1.71428571 362.271655 1.71428571 362.271655 38.2857143 368.223844 38.2857143 368.223844 39.0342857 350.990511 39.0342857"></polygon>
                                        <polygon id="Path"
                                                 points="366.822993 38.2857143 372.758029 38.2857143 372.758029 1.71428571 366.822993 1.71428571 366.822993 0.96 378.258516 0.96 396.338078 30.1028571 396.440998 30.1028571 396.440998 1.71428571 390.523114 1.71428571 390.523114 0.96 403.199392 0.96 403.199392 1.71428571 397.25292 1.71428571 397.25292 39.7714286 396.183698 39.7714286 373.667153 3.90285714 373.558516 3.90285714 373.558516 38.2857143 379.493552 38.2857143 379.493552 39.0342857 366.822993 39.0342857"></polygon>
                                        <path
                                            id="Path"
                                            d="M441.411192,25.3885714 L437.134307,25.3885714 L437.134307,37.52 L436.493917,37.52 C435.744891,36.72 435.424696,36.5085714 434.349757,36.5085714 C431.782482,36.5085714 429.483942,39.8171429 422.748418,39.8171429 C417.716788,39.8171429 413.977372,38.2171429 410.340876,34.5828571 C406.406536,30.785463 404.170067,25.5633423 404.137105,20.0971429 C404.071192,14.5573462 406.302331,9.2373101 410.300852,5.4 C413.514234,2.18857143 417.631022,1.42108547e-14 422.879927,1.42108547e-14 C428.340389,1.42108547e-14 429.352433,2.08571429 431.976886,2.08571429 C433.366302,2.08571429 433.846594,1.65714286 434.378345,0.942857143 L435.18455,0.942857143 L435.18455,11.6514286 L434.612774,11.6514286 C434.040998,7.96571429 431.753893,0.748571429 422.84562,0.748571429 C412.204866,0.748571429 410.32944,10.6342857 410.32944,20.0971429 C410.32944,26.5657143 411.238564,33.0857143 415.035158,36.5085714 C417.179319,38.4285714 419.157664,39.0742857 422.794161,39.0742857 C427.231144,39.0742857 432.045499,36.1828571 432.045499,29.88 L432.045499,25.3885714 L425.115572,25.3885714 L425.115572,24.64 L441.411192,24.64 L441.411192,25.3885714 Z"></path>
                                        <path
                                            id="Path"
                                            d="M442.343187,27.5257143 L442.914964,27.5257143 C445.379319,39.1257143 451.954745,39.0742857 455.911436,39.0742857 C463.453163,39.0742857 465.380049,33.8857143 465.380049,30.4114286 C465.380049,25.44 461.206083,23.4114286 458.215693,22.5028571 L452.074818,20.6285714 C446.299878,18.8685714 443.303771,16.0571429 443.303771,11.2228571 C443.303771,4.75428571 448.118127,0 455.013747,0 C459.9882,0 461.377616,2.08571429 463.413139,2.08571429 C464.322263,2.08571429 464.48236,1.97714286 465.01983,0.942857143 L465.763139,0.942857143 L465.763139,10.6342857 L465.122749,10.6342857 C464.270803,5.49142857 461.120316,0.748571429 455.013747,0.748571429 C451.537348,0.748571429 446.191241,2.40571429 446.191241,8.06857143 C446.191241,12.6685714 450.193674,14.2171429 453.893066,15.3942857 L460.365572,17.4228571 C463.035766,18.28 469.51399,19.8857143 469.51399,27.2057143 C469.51399,33.8342857 464.368005,39.8171429 455.928589,39.8171429 C453.65555,39.8996587 451.383437,39.6297925 449.193066,39.0171429 C447.80365,38.5885714 445.985401,37.84 444.750365,37.84 C443.681144,37.84 443.418127,38.3257143 443.149392,39.0171429 L442.348905,39.0171429 L442.343187,27.5257143 Z"></path>
                                    </g>
                                </g>
                            </g>
                        </g>
                    </svg>
                </NuxtLink>
            </div>
        </header>
        <Nuxt/>
        <footer class="amp-footer">
            <SocialLinks class="footer__social-links" lightIcons/>
            <div class="footer-navigation">
                <NuxtLink class="link" to="/about/">About</NuxtLink>
                <NuxtLink class="link" rel="nofollow" to="/advertise/">Advertise</NuxtLink>
                <NuxtLink class="link" to="/contact/">Contact</NuxtLink>
                <Link
                    to="/newsletter/"
                    force-no-amp
                    class="link"
                    rel="nofollow"
                >
                    Subscribe to newsletter
                </Link>
                <Link
                    to="/wedding/submit/"
                    force-no-amp
                    class="link"
                    rel="nofollow"
                >
                    Submit a Wedding
                </Link>
                <NuxtLink class="link" rel="nofollow" to="/work-with-us/">
                    Are You a Business?
                </NuxtLink>
            </div>
            <div class="copyright-description">
                <div class="company">© {{ currentYear }} INSIDE WEDDINGS</div>
                <div>
                    <NuxtLink class="terms-of-use" rel="nofollow" to="/terms-of-use">
                        TERMS OF USE
                    </NuxtLink>
                    <NuxtLink rel="nofollow" to="/privacy-policy"> PRIVACY POLICY</NuxtLink>
                </div>
            </div>
        </footer>
        <amp-iframe frameborder="0" height="1"
                    sandbox="allow-scripts"
                    src="https://cdn.jsdelivr.net/npm/prebid-universal-creative@latest/dist/load-cookie.html"
                    title="User Sync"
                    width="1">
            <amp-img layout="fill"
                     placeholder
                     src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw=="></amp-img>
        </amp-iframe>
    </amp-body>
</template>

<script>
import pageLinks from '~/constants/ampPageLinks'
import {NEWS_CATEGORIES} from "~/constants/newsCategories";
import icons from "~/assets/icons";

export default {
    name: "amp",
    data: () => ({
        pageLinks,
        NEWS_CATEGORIES
    }),
    head() {
        return {
            script: [
                {
                    src: 'https://cdn.ampproject.org/v0/amp-analytics-0.1.js',
                    'custom-element': 'amp-analytics',
                    async: true
                },
                // {
                //     src: 'https://cdn.ampproject.org/v0/amp-carousel-0.2.js',
                //     'custom-element': 'amp-carousel',
                //     async: true
                // },
                // {
                //     src: 'https://cdn.ampproject.org/v0/amp-sidebar-0.1.js',
                //     'custom-element': 'amp-sidebar',
                //     async: true
                // }
            ]
        }
    },
    computed: {
        currentYear() {
            return new Date().getFullYear()
        }
    },
    methods: {
        getIcon(name) {
            return icons[name]
        },
        linkToCategory(categorySlug) {
            const baseLink = {
                name: 'news-category',
                params: {
                    category: categorySlug
                }
            }
            const path = this.$router.resolve(baseLink).href
            return `/amp${path}`
        },
    }
}
</script>
